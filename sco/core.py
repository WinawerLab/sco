...
def _check_names(names, where):
    for name in names:
        if not isinstance(name, basestring):
            raise ValueError(
                'value %s from %s must be an identifier' % (name, where))
    return names
...
def _argspec_expects(argspec):
    ...
    if vargs is not None:
        raise ValueError(
            'calculation/initialize/validate functions may not take variable args')
...
def _provision(f):
    ...
    outputs = set(f.expectation if hasattr(
        f, 'expectation') else _expectation(f))
    if hasattr(f, 'provision'):
        outputs.update(f.provision)
    return outputs
...
def _prep_argspec(argspec, data_pool):
    ...
    args = [data_pool[arg] for arg in argnames] + \
           [(data_pool[arg] if arg in data_pool else val)
            for (arg, val) in zip(dfltargs, defaults)]
...
def _merge_dictargs(dictargs, kwargs):
    ...
    for da in dictargs:
        if not isinstance(da, DictType):
            raise ValueError(
                'Calculations only accepts dictionaries and keywords as parameters')
        data_pool.update(da)
...
def _calculator_call(f, argspec, names, expc, prov, args, kwargs):
    ...
    if len(missing) > 0:
        raise ValueError('Expected arguments missing: %s' % (missing,))
    (params, kwparams) = _prep_argspec(argspec, data_pool)
    rval = f(*params, **kwparams)
    if not isinstance(rval, DictType):
        raise ValueError(
            'Function with @provides decorator failed to return dict')
    for name in names:
        if name not in rval:
            raise ValueError(
                'Function @provides %s but failed to return it' % name)
...
def calculates(*names, **argtr):
    ...
    if not all(isinstance(name, basestring) for name in names):
        raise ValueError(
            '@provides decorator requires strings as the arguments')
...
def calc_chain(*args, **kw):
    '''calc_chain(a, b, ...) yields a callable calculation module object

    that incorporates the given calculation functions a, b, etc. into a
    chain of calculations. The calculations are run in order, and any of
    the arguments may be either tuples or lists, in which case these are
    flattened. A tuple (string, calc) is considered a calc with a name;
    a calc not appearing in such a pair will be given the name according
    to it's __name__ attribute.  A sequence of 1 or more dictionary
    arguments may follow the calculation objects, as well as a sequence
    of keywords, all of which will be merged left-to-right into a single
    dictionary and used to replace the elements of the given chain; the
    keys in the replacement dictionary may be either the calculations to
    replace themselves or their names, and the replacements must also be
    calculations.
    '''
    calc_layers = []
    repl = None
    ...
    if len(calc_layers) == 0:
        raise ValueError('calc_chain() requires at least one argument.')
    # Make replacements
    ...
    for (nm, m) in calc_layers:
        if not iscalc(m):
            raise ValueError(
                'given object %s (%s) is not a calculation!' % (nm, m))
    # Calculate the expectation and provision;
    # The expectation is tricky: it's everything expected prior to being
    # provided
    prov_net = set([])
    expc_net = set([])
    opts_net = {}
...
def calc_translate(calc, *args, **kw):
    '''Translates expectations and provisions of calc to given replacements

    calc_translate(calc, replacements...) yields a version of the
    calculation calc in which the expectations and provisions have been
    changed to the given replacements but which otherwise functions
    identically. If calc is a list or tuple, then it is passed to
    calc_chain first.  The replacements arguments may be supplied as
    zero 0 or more dictionaries of replacements followed by zero or more
    keyword arguments. These are merged left-to-right.
    '''
    if isinstance(calc, (tuple, list)):
        calc = calc_chain(calc)
    ...
    def _calc_translate(*dictargs, **kwargs):
        ...
        return rval
    _calc_translate.expectation = set(
        [itr[k] if k in itr else k for k in calc.expectation])
    _calc_translate.provision = set(
        [itr[k] if k in itr else k for k in calc.provision])
