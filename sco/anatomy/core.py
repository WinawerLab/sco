...
# anatomy/core.py
#
# The anatomy module of the standard cortical observer; core
...
@calculates('pRF_centers', 'pRF_voxel_indices', 'pRF_polar_angle',
...
def calc_pRFs_from_freesurfer_retinotopy(polar_angle_mgh, eccentricity_mgh,
    ...
    [hem, pRF_voxel_indices] = [np.asarray(
        [r[i] for r in tmp]) for i in [0, 1]]
...
def calc_Kay2013_pRF_sizes(pRF_eccentricity, pRF_v123_labels,
                           ...
                           Kay2013_output_nonlinearity={1: 0.18, 2: 0.13,
                                                        3: 0.12}):
    '''calculate_pRF_sizes_Kay2013(pRF_eccentricity, pRF_v123_labels)

    yeilds a list of pRF sizes for the given lists of eccentricity
    values and labels (which should all be 1, 2, or 3 for V1-V3,
    otherwise 0). For any label that isn't 1, 2, or 3, this function
    returns 0.  sigma0(p, a) = s0 + sa (p/90) where sa is a constant
    determined by the visual area and s0 is 1/2; the values for sa given
    in Kay et al. (2013) are {s1 = 0.1, s2 = 0.15, s3 = 0.27}.  sigma(p,
    a, n) = (sigma0(p, a) - 0.23) / (0.16 n^(1/2) + -0.05)
    '''
    ...
    sig = [0 if l not in Kay2013_output_nonlinearity else
           (s0 - 0.23) /
           (0.16 / np.sqrt(Kay2013_output_nonlinearity[l]) - 0.05)
...
def export_predicted_responses(export_path,
    ...
    for (nm, result) in zip(predicted_response_names, predicted_responses):
        ...
        if os.path.isdir(flname):
            raise ValueError('Output filename %s is a directory!' % flname)
        if os.path.exists(flname) and not overwrite_files:
            continue
        if len(result) != len(pRF_voxel_indices):
            raise ValueError(
                'data (%s) is not the same length as pRF_voxel_indices!' % nm)
        ...
        mgh = fs.mghformat.MGHImage(
            arr, vol0.affine, hdr, vol0.extra, vol0.file_map)
