...
Kay2013_pRF_output_nonlinearities = {1: 0.18, 2: 0.13, 3: 0.12}
    
@calculates('pRF_sizes')
def calculate_pRF_sizes_Kay2013(pRF_eccentricity, pRF_v123_labels):
    '''
    ...
    '''
    sig0 = [0.5 + (Kay2013_pRF_sigma_slope[l] if l in Kay2013_pRF_sigma_slope else 0) * e/90
            for (e, l) in zip(pRF_eccentricity, pRF_v123_labels)]
    sig  = [0 if l not in Kay2013_pRF_output_nonlinearities else \
            (s0 - 0.23) / (0.16 * math.sqrt(Kay2013_pRF_output_nonlinearities[l]) - 0.05)
            for (e, l, s0) in zip(pRF_eccentricity, pRF_v123_labels, sig0)]
    return {'pRF_sizes': sig}


@calculates()
def export_predictions(export_path, predictions, pRF_voxel_indices, subject,
    ...
    for (nm, result) in predictions.iteritems():
        ...
        for ((i,j,k),val) in zip(voxel_indices, result):
            arr[i,j,k] = val
