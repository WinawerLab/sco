...
# stimulus/core.py
#
# The stimulus module of the standard cortical observer; core
...
def import_stimulus_images(filenames):
    '''import_stimulus_images is a calculator that expects the

    'stimulus_image_filenames' value and converts this, which it expects
    to be a list of image filenames (potentially containing pre-loaded
    image matrices), into a list of images loaded from those
    filenames. It provides this list as 'stimulus_images'.
    '''
    ims = filenames if hasattr(filenames, '__iter__') else [filenames]
    ims = [img_as_float(data.load(im) if isinstance(
        im, basestring) else im) for im in ims]
...
def calc_normalized_stimulus_images(imgs, edge_val=0.0, deg2px=24, normsz=(300, 300), normdeg2px=15):
    ...
    '''
    if not hasattr(edge_val,   '__iter__'):
        edge_val = [edge_val for i in imgs]
    if not hasattr(deg2px,     '__iter__'):
        deg2px = [deg2px for i in imgs]
...
def calc_gabor_filters(d2p, octaves=4, steps=2, orientations=4, minf=1):
    ...
    if not hasattr(orientations, '__iter__'):
        orientations = np.asarray(
            range(orientations), dtype=np.float) * math.pi / orientations
    wlt_freqs = np.asarray([2.0**(float(q) / float(steps))
                            for q in range(octaves * steps + 1)])
